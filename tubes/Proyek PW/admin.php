...
if(isset($_GET["A-Z"])){
  ...
  $query = "SELECT * FROM `murid` WHERE `kelas` ORDER BY `nama` ASC";
  $murid= query($query);
}
...
if(isset($_GET["Z-A"])){
  ...
  $query = "SELECT * FROM `murid` WHERE `kelas` ORDER BY `nama` DESC";
  $murid= query($query);
}

if(isset($_GET["1-9"])){
  
  $query = "SELECT * FROM `murid` WHERE `kelas` ORDER BY `umur` ASC";
  $murid= query($query);
}

if(isset($_GET["9-1"])){
  
  $query = "SELECT * FROM `murid` WHERE `kelas` ORDER BY `umur` DESC";
  $murid= query($query);
}
...
<html >
  <head>
    ...
    <title>Proyek LATKUS Admin</title>
  </head>
  <body>
    <!-- navbar -->
        ...
        <a class="navbar-brand" href="#"> <img class="logonav" src="asset/logov2.png" height="40px" alt="" style="text-align: center;"></a>
          ...
          <div class="collapse navbar-collapse " id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
              ...
              <li class="nav-item">
                <a class="nav-link" href="admin.php">Data Murid</a>
              </li>
              ...
              <?php $i = 1; foreach($murid as $mrd) : ?>
                <tr>
                  ...
                  <td class="align-middle ">
                    <a href="profil.php?id_murid=<?= $mrd["id_murid"] ?>">
                  ...
                  <td class="align-middle aksi">
                    ...
                    <a href="ubah.php?id_murid=<?= $mrd["id_murid"] ?>" class="btn badge bg-primary">Ubah</a>
                    <a href="hapus.php?id_murid=<?= $mrd["id_murid"] ?>" class="btn badge bg-danger" onclick="return confirm('Yakin Akan Dihapus?')">Hapus</a>
                  </td>
                </tr>
                <?php endforeach; ?>
